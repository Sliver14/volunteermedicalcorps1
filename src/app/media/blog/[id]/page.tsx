...
export default async function BlogDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="w-full bg-white font-roboto">
      <PageBanner title={blog.title} parent={{ label: "Blog", href: "/media/blog" }} />
        ...
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6">
          <div className="flex flex-col lg:flex-row gap-12">
            ...
            <div className="lg:w-2/3">
              <Link href="/media/blog" className="inline-flex items-center text-[#002866] font-bold text-sm mb-8 hover:text-[#ff9f22] transition-colors">
                  ...
                  {recentBlogs.map((item) => (
                    <li key={item.id} className="flex items-center gap-4 group cursor-pointer">
                      <div className="relative w-20 h-20 shrink-0 overflow-hidden rounded-sm">
                        <Image src={item.image || "https://volunteermedicalcorps.org/admin/images/media/en9p2Ej1Q341529768.jpg"} alt={item.title} fill className="object-cover group-hover:scale-110 transition-transform duration-500" unoptimized />
                      </div>
                      <div>
                        <h4 className="font-bold text-[#002866] text-sm group-hover:text-[#ff9f22] transition-colors leading-tight mb-2">
                          <Link href={`/media/blog/${item.id}`}>{item.title}</Link>
                        </h4>
