  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
        <div>
          <h2 className="text-3xl font-black text-[#002866]">Welcome back, {session.user.name.split(' ')[0]}!</h2>
          <p className="text-slate-500 font-medium">You have {enrollments.length} courses in progress. Keep up the good work!</p>
        </div>
        ...
        <div className="lg:col-span-2 space-y-12">
          ...
          <section>
            <div className="flex justify-between items-center mb-6">
              ...
              <Link href="/elearn/my-courses" className="text-sm font-bold text-[#002866] hover:underline">View My Courses</Link>
            </div>
            
            {enrollments.length === 0 ? (
                <div className="bg-white border-2 border-dashed border-slate-200 rounded-3xl p-12 text-center">
                    <p className="text-slate-400 font-medium mb-6">You haven't enrolled in any courses yet.</p>
                    <Link href="/elearn/courses" className="bg-[#002866] text-white px-8 py-3 rounded-xl font-bold text-sm">Explore Catalog</Link>
                </div>
            ) : (
                ...
                {enrollments.map((enr: any, index: number) => (
                    ...
                    <div className="relative h-40">
                        <Image 
                            src={enr.course.image || "https://volunteermedicalcorps.org/admin/images/campaigns/Sn2yMRj85138725946.jpg"} 
                        ...
                        <Link href={`/elearn/courses/${enr.course.id}`} className="w-12 h-12 bg-white rounded-full flex items-center justify-center text-[#002866]">
                            <FaPlay className="ml-1" />
                        </Link>
                        </div>
                    </div>
                    <div className="p-6">
                        <h4 className="font-bold text-slate-800 mb-1 line-clamp-1">{enr.course.title}</h4>
                        <p className="text-xs text-slate-400 font-medium mb-4">{enr.course.category.name}</p>
                        <div className="space-y-2">
                        <div className="flex justify-between text-[10px] font-black uppercase tracking-widest">
                            <span className="text-slate-400">Progress: {enr.progress}%</span>
                        </div>
            ...
            <div className="space-y-4">
              {recommendedCourses.map((course: any, index: number) => (
                <div key={index} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between group hover:border-[#ff9f22] transition-colors">
                  ...
                  <Link href={`/elearn/courses/${course.id}`} className="px-6 py-2 bg-slate-50 text-slate-600 rounded-xl text-xs font-bold group-hover:bg-[#ff9f22] group-hover:text-[#002866] transition-all">
                    Enroll Now
                  </Link>
                </div>
              ))}
          ...
          <div className="bg-[#002866] rounded-3xl p-8 text-white relative overflow-hidden">
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="text-2xl font-black">{stats.learningTime}</div>
