...
export default function MarqueeManager() {
  ...
  if (loading) return <p className="text-brand-primary dark:text-brand-secondary font-bold animate-pulse">Loading Marquee...</p>;

  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-black text-brand-primary dark:text-brand-secondary uppercase tracking-tight">Manage Marquee</h2>
        <button 
      ...
      {isEditing && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-bg-surface w-full max-w-xl rounded-sm shadow-2xl overflow-hidden animate-in zoom-in duration-300">
            ...
            <div className="p-8 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-text-muted">Link URL</label>
                  <input 
                    className="w-full border border-border-main bg-bg-base p-3 text-sm focus:outline-none focus:border-[#ff9f22]"
                    value={editForm.link || ""}
      ...
      <div className="space-y-4">
        {items.map((item) => (
          <div key={item.id} className="bg-bg-surface p-6 rounded-sm shadow-sm border border-border-main flex items-center justify-between group">
            <div className="flex items-center gap-6">
              ...
              <div>
                <p className={`text-sm font-bold truncate max-w-2xl ${item.color === 'red' ? 'text-red-600' : 'text-brand-primary dark:text-brand-secondary'}`}>
                  {item.content}
                </p>
                <p className="text-[10px] text-text-muted font-bold uppercase mt-1">Link: {item.link || "None"}</p>
              </div>
            </div>
            <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
              <button onClick={() => handleEdit(item)} className="p-2 text-brand-primary dark:text-brand-secondary hover:bg-bg-base rounded-full transition-colors"><Edit2 size={16} /></button>
              <button onClick={() => handleDelete(item.id)} className="p-2 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-full transition-colors"><Trash2 size={16} /></button>
            </div>
          </div>
        ))}
      </div>
